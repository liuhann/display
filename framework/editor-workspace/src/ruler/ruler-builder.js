...
ruler.prototype.rulerConstructor = function (_canvas, options, rulDimension) {
  ...
  const drawPoints = function () {
    ...
    const lineLengthMin = rulThickness / 2
  ...
  const draggable = (function () {
    return {
      ...
      startMoving: function (evt) {
        ...
        const eWi = parseInt(guideLine.offsetWidth)
        const eHe = parseInt(guideLine.offsetHeight)
        const cWi = parseInt(dragContainer.offsetWidth)
        const cHe = parseInt(dragContainer.offsetHeight)

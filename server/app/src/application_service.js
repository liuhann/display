const { ConflictError, BadRequestError, HttpError } = require('@gw/wind-core-http'),
    fs = require('fs'),
    debug = require('debug')('apollo:app'),
    axios = require('axios'),
    url = require('url'),
    ObjectID = require('bson-objectid'),
    copy = require('./copy_folder'),
    PATH_APP = 'apps',
    PATH_TRASH = 'trash',
    PATH_ARCHIVE = 'archived';
...
module.exports = class ApplicationService {
    ...
    async loginToGoldWind(email, pwd) {
        const params = new url.URLSearchParams({
                loginname: email,
                pwd: pwd
            }),

            result = await axios.post('https://devops.goldwind.com.cn/v2/auth/user/login', params.toString());

        if (result.data.errcode === '22001' || !result.data.data) {
    ...
    updateAppList() {
        const files = fs.readdirSync(`${this.config.appRoot}/${PATH_APP}`),
            appList = [],
            appRoot = `${this.config.appRoot}/${PATH_APP}`;
